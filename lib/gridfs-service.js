...
GridFSService.prototype.connect = function (cb) {
  ...
  if (!this.db) {
    let url;
    if (!self.options.url) {
      if (self.options.username) { //Backwards compatibility
        if (!self.options.user){
          self.options.user = self.options.username;
        }
        delete self.options.username;
      }
      url = (self.options.user && self.options.password) ?
    ...
    const validOptions = _.omit(self.options, ['username', 'password', 'host', 'port', 'database', 'name', 'connector', 'debug']);
    // connect
    MongoClient.connect(url, validOptions, (error, client) => {
      if (!error) {
...
/**
 * Delete all files that meet the condition
 */
...
GridFSService.prototype.deleteContainer = function (containerName, cb) {
  ...
  fs_files.find({ 'metadata.container': containerName }, { _id: 1 }).toArray(function (error, containerFiles) {
    ...
    for (const index in containerFiles) {
...
GridFSService.prototype.deleteFileByName = function (containerName, filename, cb) {
  ...
  fs_files.find({ 'metadata.container': containerName, 'metadata.filename': filename }, { _id: 1 })
      .toArray(function (error, containerFiles) {
        if (!containerFiles || containerFiles.length <= 0) {
          return cb(error);
        }
...
GridFSService.prototype.upload = function (containerName, req, cb) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    const gridfs = new GridFS(self.db, mongodb);
    const stream = gridfs.createWriteStream(options);
...
GridFSService.prototype.uploadWithMetadata = function (containerName, metadata, req, cb) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    // Add the container's name, the file name and the file's mime type to the metadata included by the user
...
GridFSService.prototype.download = function (fileId, res, cb) {
  ...
  collection.find({
    ...
    const gridfs = new GridFS(self.db, mongodb);
    const stream = gridfs.createReadStream({
      _id: file._id
    });
  ...
  collection.find({
    ...
    function next() {
      if (files.length > 0) {
        ...
        const fileStream = gridfs.createReadStream({_id: file._id});
...
/**
 * Downloads a list of files compressed in zip format
 * @param {{string}} filesId String with the file ids to download compressed,
 * separated by commas
 */
...
GridFSService.prototype.downloadZipFiles = function (filesId, res, cb) {
  ...
  collection.find({ '_id': { $in: Ids } }).toArray(function (error, files) {
    ...
    const filename = `files-${fecha.getFullYear()}${fecha.getMonth() + 1}${fecha.getDate()}`;
