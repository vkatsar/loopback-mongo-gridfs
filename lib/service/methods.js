...
module.exports = {
    ...
    fileUpload: async function(container, fileStream, filename, mimetype) {
        return h.fileUpload(this, container, fileStream, filename, mimetype);
    },
    ...
    downloadContainerFile: function (container, file, res, inline, cb) {
        this.getContainerFile(container, file).then(fileMeta => {
            const stream = h.fileDownload(this, fileMeta, res, inline);
            cb(null, stream);
        }).catch(error => cb(error));
    },
    ...
    // TODO: Replace "where" with "filters"
    downloadFileVersions: function (container, file, where = {}, res, cb) {
        this.getFileVersions(container, file, where).then(files => {
            if (files.length === 0) {
                throw h.statusError('No files in container.', 404);
            }
            const stream = h.zipDownload(this, files, res, file, '{$_id}_{$filename}');
            cb(null, stream);
        }).catch(error => cb(error));
    },
    ...
    downloadFileVersion: function (container, file, version, res, inline, cb) {
        this.getFileVersion(container, file, version).then(fileMeta => {
            const stream = h.fileDownload(this, fileMeta, res, inline, '{$_id}_{$filename}');
            cb(null, stream);
        }).catch(error => cb(error));
    }
}
